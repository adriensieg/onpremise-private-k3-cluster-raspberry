// Root path the app is served under (e.g. "/scanning"). Empty when at domain root.
...
function render(tasks) {
  ...
  for (const t of tasks) {
    ...
    li.querySelector(".toggle").onclick = async () => {
      await apiUpdate(t.id, { title: t.title, done: !t.done });
      load();
    };
...
if ("serviceWorker" in navigator) {
  navigator.serviceWorker
    .register(`${ROOT}/static/sw.js`, { scope: `${ROOT}/` })
    .catch(() => {});
}
